...
snakemake -p --profile hpcc-profiles/slurm/alpine --configfile example-configs/LOSH-Apr-25/config.yaml --until concat_gvcf_sections --rerun-incomplete
```
...
DIR="results/bqsr-round-0"
INPUT_FILE="bcf/all.bcf"
OUT="vcf/BANS.all.ds6x.pass-maf-0.05.SNP.vcf.gz"

mkdir -p "results/bqsr-round-0/vcf/"

#filter for PASS sites, MAF >= 0.05, and keep only biallelic SNPs, filter out sites with >20% missing data (F_MISSING > 0.2)
bcftools view --threads 8 \
    -i 'FILTER="PASS" & MAF>=0.05' \
    -v snps -m2 -M2 $DIR/$INPUT_FILE \
...
bcftools index --threads 16 $DIR/vcf/BANS.all.ds6x.pass-maf-0.05.SNP.vcf.gz
```

Checking how many SNPs are in each chromosome.
```{bash}
OUT="results/bqsr-round-0/vcf/BANS.all.ds6x.pass-maf-0.05.SNP.vcf.gz"
...
DIR="results/bqsr-round-0"
IN="vcf/BANS.all.ds6x.pass-maf-0.05.SNP.vcf.gz"
OUT="vcf/filtered_HC"
...
java -Djava.io.tmpdir=/scratch/alpine/ericacnr@colostate.edu/tmp -Xmx37g -jar /curc/sw/install/bio/gatk/4.3.0.0/gatk-package-4.3.0.0-local.jar SelectVariants \
     ...
     -V $DIR/$IN \
     -select-type SNP \
     --O $DIR/$OUT/BANS.all.ds6x.pass-maf-0.05.SNP.vcf

#the lines before remove some of the annotations I need for later filtering
java -Djava.io.tmpdir=/scratch/alpine/ericacnr@colostate.edu/tmp -Xmx37g \
  -jar /curc/sw/install/bio/gatk/4.3.0.0/gatk-package-4.3.0.0-local.jar VariantAnnotator \
  -R $REFERENCE \
  -V $DIR/$OUT/BANS.all.ds6x.pass-maf-0.05.SNP.vcf \
  -O $DIR/$OUT/BANS.all.ds6x_gatk.pass-maf-0.05.SNP.annot.vcf.gz \
...
-R $REFERENCE \
-V $DIR/$OUT/BANS.all.ds6x_gatk.pass-maf-0.05.SNP.annot.vcf.gz \
...
-O $DIR/$OUT/BANS.all.ds6x_gatk.SNP.05maf.hardfilter.vcf

#Exclude filtered variants (PASS= pass filters)
java -Djava.io.tmpdir=/scratch/alpine/ericacnr@colostate.edu/tmp -Xmx37g -jar /curc/sw/install/bio/gatk/4.3.0.0/gatk-package-4.3.0.0-local.jar  SelectVariants \
     -R $REFERENCE \
     -V $DIR/$OUT/BANS.all.ds6x_gatk.SNP.05maf.hardfilter.vcf \
     --exclude-filtered  true \
     -O $DIR/$OUT/BANS.all.ds6x_gatk.SNP.05maf.hardfilter.PASS.vcf
```

Optional initial missingness filtering, but see below for individual level filtering first.
```{bash}
conda activate bioinf
...
vcftools --vcf $DIR/$OUT/BANS.all.ds6x_gatk.SNP.05maf.VQSR2.PASS.vcf --out $DIR/$OUT/BANS.all.ds6x.pass-maf-0.05.bialSNP.filtered.0.8miss.recode.vcf --max-missing 0.8 --recode --recode-INFO-all
```

## filtering for individual level missingness before loci level 

From within vcf_file. Will this give me more loci? Theoretically, having high missing indv would bias the missingness at loci level to a higher level so they would get filtered out. So dropping high indv missing would result in more snps.

<!-- After filtering, kept 229 out of 229 Individuals -->
<!-- Outputting Individual Missingness -->
<!-- After filtering, kept 3124563 out of a possible 3124563 Sites -->

```{bash}
vcftools --gzvcf BANS.all.ds6x.pass-maf-0.05.SNP.vcf.gz --missing-indv
# filter out those individuals missing more than 30% of genome wide information
<!-- After filtering, kept 229 out of 229 Individuals -->
<!-- Outputting Individual Missingness -->
<!-- After filtering, kept 5219653 out of a possible 5219653 Sites -->

#filt ind with less than 30% overall missingness
awk 'NR>1 && $5 > 0.3 {print $1}' out.imiss > bad_ind
wc -l bad_ind

# then filter SNP level missingness
vcftools --gzvcf BANS.all.ds6x.pass-maf-0.05.SNP.vcf.gz --remove bad_ind --out filtered_HC/BANS.all.ds6x.pass-maf-0.05.bialSNP.filtered.ind03miss.0.8miss --max-missing 0.8 --recode --recode-INFO-all

<!-- After filtering, kept 206 out of 229 Individuals -->
<!-- Outputting VCF file... -->
<!-- After filtering, kept 3124151 out of a possible 3124563 Sites -->

vcftools --vcf BANS.all.ds6x.pass-maf-0.05.bialSNP.filtered.ind03miss.0.8miss.recode.vcf --missing-indv

bcftools stats BANS.all.ds6x.pass-maf-0.05.bialSNP.filtered.ind03miss.0.8miss.recode.vcf > BANS.all.ds6x.pass-maf-0.05.bialSNP.filtered.ind03miss.0.8miss.recode.stats

awk 'NR>1 {sum += (1-$5)} END {print sum/NR}' out.imiss
#0.883732
```

That is, in fact, what happened! 3124151 with this type of filtering. 209 samples left.
